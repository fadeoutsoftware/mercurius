...
public class SmsResource {
	...
	protected String getFinalMessage(Message oMessage) {
		...
		sFinalText+=oMessage.getMessage();
	...
	@Path("/group/{idgroup}")
	public PrimitiveResult sendToGroup(@Context HttpServletResponse oResponse, Message oMessage, @PathParam("idgroup") int idGroup,  @DefaultValue("3") @QueryParam("retry") int iRetry) {
		...
		for (Contact oContact : oGroup.getMembers()) {
			...
			oForward.setAddress(oContact.getMobile());
			oForward.setFinalText(sFinalText);
			oForward.setIdRecipient(oContact.getIdContact());
			oForward.setMedia("SMS");
			oForward.setMaxRetry(iRetry);
			oForward.setRetryCount(0);
	...
	/**
	 * Sends an SMS to a contacts list
	 ...
	 * @param iRetry Optional Retry count (Query Parameter)
	 * @return Message Id or -1
	 */
	@PUT
	@Produces({"application/xml", "application/json", "text/xml"})	
	@Consumes({"application/xml", "application/json", "text/xml"})
	@Path("/contacts")
	public PrimitiveResult sendToContacts(@Context HttpServletResponse oResponse, Message oMessage, @DefaultValue("") @QueryParam("contactsids") String sContactsIds,  @DefaultValue("3") @QueryParam("retry") int iRetry) {
		// Prepare result
		PrimitiveResult oResult = new PrimitiveResult();
		oResult.IntValue = -1;
	...
	/**
	 * Sends SMS directly to some ";" separated numbers
	 * @param oMessage Message Entity (POST Paramter)
	 * @param sNumbers String ";" sperated with target numbers (Query Parameter)
	 * @param iRetry Retry Count Optional (Query Paramter)
	 * @return Message Id or -1
	 */
	@PUT
	@Produces({"application/xml", "application/json", "text/xml"})	
	@Consumes({"application/xml", "application/json", "text/xml"})
	@Path("/direct")
	public PrimitiveResult sendToNumbers(@Context HttpServletResponse oResponse, Message oMessage, @DefaultValue("") @QueryParam("numbers") String sNumbers,  @DefaultValue("3") @QueryParam("retry") int iRetry) {
		// Prepare result
		PrimitiveResult oResult = new PrimitiveResult();
		oResult.IntValue = -1;
